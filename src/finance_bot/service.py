...
class FinanceService:
        ...
        async with self._user_locks[telegram_user_id]:
            ...
            try:
                if not force_duplicate:
                    duplicates = await self.state.find_duplicates(
                        telegram_user_id,
                        transactions,
                ...
                await self.state.append_transactions(transactions)
    ...
    async def report(self, telegram_user_id: int, period: ReportPeriod) -> ReportSummary:
        ...
        transactions = await self.state.list_transactions_between(telegram_user_id, start, end)
        return build_report(transactions, period=period, start=start, end=end)

    async def get_transactions(
        self, telegram_user_id: int, period: ReportPeriod
    ) -> tuple[list[Transaction], datetime, datetime]:
        """Return raw transactions and time bounds for a period (for table image)."""
        now = datetime.now(self.timezone)
        start, end = report_bounds(now, period)
        transactions = await self.state.list_transactions_between(telegram_user_id, start, end)
    ...
    async def undo(self, telegram_user_id: int) -> Transaction | None:
        async with self._user_locks[telegram_user_id]:
            return await self.state.undo_last(telegram_user_id)
