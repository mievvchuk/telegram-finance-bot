...
async def run(settings: Settings | None = None) -> None:
    ...
    sheets = GoogleSheetsRepository(
        ...
        worksheet_title=config.transactions_worksheet,
    )
    ...
    service = FinanceService(parser=parser, sheets=sheets, state=state, settings=config)
    bot = Bot(
    ...
    if config.default_google_sheet_id:
        await sheets.ensure_schema(config.default_google_sheet_id)
    await bot.set_my_commands(
        [
            BotCommand(command="start", description="Підключення й коротка інструкція"),
