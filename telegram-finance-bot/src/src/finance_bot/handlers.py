...
from .service import ConfirmKind, FinanceService, IngestKind, IngestOutcome, NotConnectedError
...
async def _connection_hint(message: Message, service: FinanceService) -> None:
    email = service.service_account_email or "client_email з service-account JSON"
    await message.answer(
        "Спочатку підключи Google Таблицю:\n"
        f"1. Надай <code>{escape(email)}</code> доступ Редактора.\n"
        "2. Надішли <code>/connect URL_АБО_ID</code>."
    )
...
def create_router(service: FinanceService, settings: Settings) -> Router:
    ...
    async def start(message: Message, state: FSMContext) -> None:
        ...
        sheet_id = await service.get_sheet_id(user_id)
        if sheet_id:
            await message.answer(
                ...
                f"✅ Таблиця підключена: <code>{escape(sheet_id)}</code>"
            )
            return
        email = service.service_account_email or "client_email з service-account JSON"
        await message.answer(
            "Привіт! Я перетворюю повідомлення й фото чеків на записи в Google Sheets.\n\n"
            "Щоб почати:\n"
            f"1. Створи Google Таблицю й надай <code>{escape(email)}</code> "
            "доступ Редактора.\n"
            "2. Надішли <code>/connect URL_АБО_ID</code>.\n\n"
            "Після цього просто пиши: <i>таксі додому 180</i>."
        )

    @router.message(Command("connect"))
    async def connect(message: Message, command: CommandObject) -> None:
        if not command.args:
            await message.answer(
                "Додай посилання або ID: <code>/connect "
                "https://docs.google.com/spreadsheets/d/…/edit</code>"
            )
            return
        status = await message.answer("Перевіряю доступ і структуру таблиці…")
        try:
            sheet_id = await service.connect_sheet(_user_id(message), command.args.strip())
        except ValueError:
            await status.edit_text("Не впізнав URL або ID Google Таблиці. Перевір посилання.")
        except Exception:
            logger.exception("Could not connect spreadsheet", extra={"user_id": _user_id(message)})
            email = service.service_account_email or "service account"
            await status.edit_text(
                "Не вдалося відкрити таблицю. Перевір, що вона існує та має "
                f"доступ Редактора для <code>{escape(email)}</code>."
            )
        else:
            await status.edit_text(format_connected(sheet_id))
    ...
    async def report(message: Message) -> None:
        try:
            ...
            await _connection_hint(message, service)
        except Exception:
            logger.exception("Could not build report", extra={"user_id": _user_id(message)})
            await message.answer("Не вдалося прочитати таблицю для звіту. Спробуй трохи пізніше.")
        else:
    ...
    async def undo(message: Message) -> None:
        try:
            ...
            await message.answer("Не вдалося видалити запис. Перевір доступ до таблиці.")
        else:
            if transaction is None:
                await message.answer("Немає записів бота, які можна скасувати.")
            else:
                await message.answer(
                    "↩️ Видалено: "
    ...
    async def transaction_callback(query: CallbackQuery) -> None:
        ...
        if action in {"ok", "force"}:
            ...
            if result.kind == ConfirmKind.DUPLICATE and result.draft:
                ...
                await query.message.edit_text(f"✅ Записано {noun} в Google Таблицю.")
